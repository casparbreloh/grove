...
- Validate before mutation. Preserve rollback, alternate-index archival, private modes, advisory locking, conservative integration detection, compare-and-delete behavior, and recoverable `closing` state; destructive changes require a real-Git regression test.
- Pi JSONL is agent-owned and must remain byte-for-byte untouched by Grove. Grove-owned records and artifacts must be atomic and private.
