...
- [`packages/coding-agent`](https://github.com/earendil-works/pi/tree/6c87d9a026677b601e8278030dcf1ad97fe0bd86/packages/coding-agent) is the product composition Grove should run for local and development-machine Agents. Its SDK and extension system preserve Pi's model/auth, tools, compaction, conversation tree, and official TUI. Its current harness factory also demonstrates wrapping injected execution capabilities with coding-agent prompt contributions and default read/bash/edit/write tools. [Coding-agent harness composition](https://github.com/earendil-works/pi/blob/6c87d9a026677b601e8278030dcf1ad97fe0bd86/packages/coding-agent/src/server/create-harness.ts#L80-L159)

For Grove, use Pi Coding Agent directly through its SDK and typed extensions wherever the host supports it. Grove still owns Task, Environment, revision, journal, receipt, and client-projection semantics; Pi owns the behavior and private continuation state of a Pi Session. Treat `ExecutionEnv` and tool factories as the Cloudflare capability seam, bound to Grove Environment operations. Study Pi's protocol/client/server work for development-machine transport, but do not expose it as Grove's public product model while it is experimental.
...
6. Provider/model/auth behavior and Pi continuation state belong to Pi; Project/Task durability, Environment ownership, and Grove's client projection remain Grove's responsibility.
