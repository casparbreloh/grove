# Grove sidebar material research

Date: 2026-08-23

Status: reference for the selected Ocean-tinted shadcn glass treatment

## Conclusion

Grove uses one fixed Ocean-tinted shadcn glass surface for its sidebar and continuous header
region. The renderer tints the opaque popover surface, applies 70% glass coverage, and places it
over Electron's semantic macOS sidebar material. Future Workspaces may choose another restrained
tint from the documented palette, but material selection is not a user-facing setting.

Electron exposes no sidebar-vibrancy intensity control. The native substrate and the shadcn CSS
surface are complementary layers: Electron samples the desktop, while the renderer owns the
stable color, opacity, blur, and semantic tokens above it. This requires no renderer-to-main IPC
and preserves Grove's renderer boundary.
...
Therefore the honest implementation is one semantic `sidebar` material with a colored CSS overlay. The overlay changes how much desktop color remains visible; it does not change the native blur kernel or create a second AppKit material intensity.
...
Keep the main inset opaque. The header's leading region and the sidebar consume the same Ocean
surface token so they remain one visual plane. The surface tints the opaque shadcn popover color
first and then applies the shared 70% glass coverage.

Preserve the current mostly grayscale OKLCH scheme:
...
2. Keep the content inset opaque. Only the sidebar and its continuous header region use the glass surface.
3. Keep the material in semantic CSS tokens rather than a new component primitive or renderer state.
4. On non-macOS platforms, use the same CSS surface over the opaque window fallback; do not claim native macOS vibrancy there.
5. Keep dropdowns and popovers on the preset's ordinary opaque `popover` surface rather than inheriting sidebar glass.
6. When Workspace appearance becomes product state, vary only the low-chroma tint anchor and keep the material recipe shared.
...
- the translucent sidebar should resolve to its opaque semantic fallback in CSS.
...
Validate the selected treatment under all of the following:
