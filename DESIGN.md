...
- Type: `text-ui-micro` is 12/16 metadata, `text-ui-small` is 13/19 controls and compact UI, `text-ui` is 15/24 reading and composition, `text-section-title` is 24/32, and `text-page-title` is 56/61. Apply roles at primitive defaults before local overrides.
...
Custom CSS is justified only for platform layout variables, surface-independent masks, or behavior a primitive cannot express clearly. Scroll fades and title clipping use alpha masks so they work over opaque and translucent surfaces; fades update their endpoints from actual scroll state. Never emulate them with black or white surface gradients. Preserve message `content-visibility`, native panel resizing, and CSS-driven pane animation rather than moving high-frequency work into React state.
