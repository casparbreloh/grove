...
- **Repository directory** — `~/.grove/repositories/<repository-name>-<path-hash>/`, deterministically identified from the canonical Git common directory and verified by `repository.json`.
...
- One Change activity lock prevents concurrent managed Pi writers and any removal while Pi is open; a separate metadata lock serializes atomic `change.json` updates.
