...
fn ship(git: &Git) -> Result<()> {
    ...
    let forge = forge::Forge::from_remote(&target.url)?;
    let base = forge.preflight()?;
    let base_ref = git.fetch_ship_base(&selected.worktree_path, &target.remote, &base)?;
    let existing = forge.find_open(&branch)?;
    let published = existing.is_some()
        || git.remote_branch_exists(&selected.worktree_path, &target.remote, &branch)?;
    if !published && !git.has_ship_work(&selected.worktree_path, &base_ref)? {
        bail!("Change has no work to ship");
    }

    git.prepare_ship(&selected.worktree_path, &branch)?;
    let context = git.ship_context(&selected.worktree_path, &base_ref)?;
    ...
    git.validate_ship_snapshot(&selected.worktree_path, &branch, &context)?;
    if context.staged {
        ...
        git.commit_ship(&selected.worktree_path, subject, &context)?;
    }
    git.validate_clean_ship(&selected.worktree_path)?;
    git.push_ship(&selected.worktree_path, &target.remote, &branch)?;
